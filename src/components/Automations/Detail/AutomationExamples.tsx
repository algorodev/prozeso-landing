        ...
        <div className='grid md:grid-cols-2 gap-6'>
          {automation.examples.map((ex: any, idx: number) => (
            <div key={idx} className='rounded-3xl border border-border bg-card p-6 flex flex-col gap-3'>
              <h3 className='font-semibold text-lg'>{t(`examples.${idx}.title`, { default: ex.title })}</h3>
              <p className='text-sm text-muted-foreground leading-relaxed'>{t(`examples.${idx}.scenario`, { default: ex.scenario })}</p>
              {ex.result && (
                <div className='mt-2 inline-flex items-center gap-2 rounded-xl bg-chart-2/10 px-4 py-2 text-sm'>
                  ...
                  <span>{t(`examples.${idx}.result`, { default: ex.result })}</span>
                </div>
              )}
            </div>
          ))}
