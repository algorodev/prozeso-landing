...
export default function AutomationsBackground() {
	...
	const shapes = useMemo<ShapeConfig[]>(
		() => [
			...
			{ id: 'a3', type: 'diamond', size: 160, top: '60%', left: '10%', opacity: 0.22, blurClass: 'blur-xl', depth: 1 },
			{
				id: 'a4',
				type: 'square',
				size: 140,
			...
			{ id: 'a5', type: 'triangle', size: 130, top: '72%', left: '78%', opacity: 0.3, blurClass: 'blur-lg', depth: 2 },
			{ id: 'a6', type: 'circle', size: 120, top: '36%', left: '18%', opacity: 0.45, blurClass: 'blur-md', depth: 3 },
			{
				id: 'a7',
				type: 'pill',
				size: 160,
			...
			{ id: 'a8', type: 'hex', size: 95, top: '80%', left: '50%', opacity: 0.35, blurClass: 'blur-sm', depth: 3 },
			{ id: 'a9', type: 'square', size: 90, top: '40%', left: '84%', opacity: 0.3, blurClass: 'blur-sm', depth: 3 },
		],
		[],
	)
	...
	return (
		<div ref={containerRef} className='absolute inset-0 -z-10 overflow-hidden'>
			...
			{shapes.map(({ id, type, size, top, left, opacity, blurClass, depth, rotation }) => (
				<div
					key={id}
					data-geo
					data-depth={depth}
					...
					style={{
						...
						left,
						width: size,
						height: type === 'pill' ? Math.max(60, Math.floor(size * 0.4)) : size,
						opacity,
						transform: rotation ? `rotate(${rotation}deg)` : undefined,
						borderRadius: type === 'circle' ? '9999px' : type === 'pill' ? '9999px' : undefined,
						clipPath: type !== 'circle' && type !== 'pill' ? clipPathFor(type) : undefined,
			...
			<span data-connector className='absolute block opacity-20 pointer-events-none'
			      style={{
				      top: '18%',
				      left: '28%',
				      width: 220,
