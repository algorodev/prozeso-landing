...
import { Stat, STATS } from '@/app/[locale]/automations/[id]/data'
...
export default function StatsMetric({ automationId }: { automationId: string }) {
			...
			<div className='mx-auto max-w-7xl px-6 py-10 sm:px-8'>
				...
				<TooltipProvider>
					<div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
						{stats.map((s, i) => (
							<MetricCard key={s.id} stat={s} automationId={automationId} index={i} />
						))}
					</div>
				</TooltipProvider>
			</div>
...
function MetricCard({ stat, index, automationId }: { stat: Stat; index: number; automationId: string; }) {
	const pct = stat.target ? Math.min(100, Math.round((stat.value / stat.target) * 100)) : 100;
...
function Ring({ value, palette = "primary", icon }: { value: number; palette?: Stat["palette"]; icon: ComponentType<{ className?: string }> }) {
	const angle = Math.round((value / 100) * 360);
	const tint = palette === "primary" ? "var(--primary)" : palette === "accent" ? "var(--accent)" : "var(--muted-foreground)";
	const Icon = icon;
...
function SparkBars({ data, palette = "primary" }: { data: number[]; palette?: Stat["palette"] }) {
	const tint = palette === "primary" ? "var(--primary)" : palette === "accent" ? "var(--accent)" : "var(--muted)";
	return (
		<div className="flex h-10 justify-center items-end gap-1">
			{data.map((v, i) => (
				<motion.div
					...
					style={{ height: `${Math.max(0.1, v) * 100}%`, backgroundColor: tint, opacity: 0.9 }}
				/>
			))}
		</div>
	);
