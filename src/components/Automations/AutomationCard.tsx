...
export default function AutomationCard({ automation }: { automation: AutomationItem }) {
	...
	const description = tItem.has('description') ? tItem('description') : automation.description
	const metrics: string[] = (tItem.has('metrics') ? (tItem.raw('metrics') as unknown) : automation.metrics) as
		| string[]
		| undefined as any
	...
	return (
		<LocalizedLink key={automation.id} href={`/automations/${automation.id}`} className='block h-full'>
			<div
				className='group h-full flex flex-col cursor-pointer rounded-2xl border-2 border-border p-4 transition-all hover:border-primary hover:scale-[1.05] relative overflow-hidden'>
				<div
