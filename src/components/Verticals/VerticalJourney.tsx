          ...
          {steps.map((step, idx) => (
            <div key={idx} className="flex gap-6">
              <div className="flex flex-col items-center">
                ...
                {idx < steps.length - 1 && (
                  <div className="w-px h-full bg-chart-2 min-h-[60px]" />
                )}
              </div>
              <div className="pb-8">
                <h3 className="font-bold text-base mb-2">
                  {id && t.has(`verticals.${id}.journey.steps.${idx}.stage`)
                    ? (t(
                        `verticals.${id}.journey.steps.${idx}.stage`,
                      ) as string)
                    : step.stage}
                </h3>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  {id &&
                  t.has(`verticals.${id}.journey.steps.${idx}.description`)
                    ? (t(
                        `verticals.${id}.journey.steps.${idx}.description`,
                      ) as string)
                    : step.description}
                </p>
              </div>
