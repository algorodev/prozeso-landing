            ...
            <CardContent className="pt-2">
              {!reportGenerated ? (
                                    ...
                                    <DropdownMenu>
                                      <DropdownMenuTrigger asChild>
                                        <Button
                                          ...
                                          aria-label={t("field.companySize.label")}
                                        >
                                          <span className="truncate flex-1 min-w-0 text-left">
                                            {field.value && COMPANY_SIZE_KEYS.includes(field.value as CompanySizeKey)
                                              ? t(`field.companySize.options.${field.value}`)
                                              : t("field.companySize.placeholder")}
                                          </span>
                                          <ChevronDown
                                        ...
                                        {COMPANY_SIZE_KEYS.map((key) => (
                                          ...
                                          >
                                            <Check
                                              ...
                                              aria-hidden
                                            />
                                            {t(`field.companySize.options.${key}`)}
                                          </DropdownMenuItem>
                                        ))}
                                      </DropdownMenuContent>
                                    </DropdownMenu>
                                    ...
                                    <DropdownMenu>
                                      <DropdownMenuTrigger asChild>
                                        <Button
                                          ...
                                          variant="outline"
                                          className="w-full min-w-0 bg-input/30 rounded-md justify-between font-normal overflow-hidden"
                                          aria-label={t("field.industry.label")}
                                        >
                                          <span className="truncate flex-1 min-w-0 text-left">
                                            {field.value && INDUSTRY_KEYS.includes(field.value as IndustryKey)
                                              ? EXISTING_VERTICALS.includes(field.value as IndustryKey)
                                                ? tSolutions(`${field.value}.title`)
                                                : t(`field.industry.options.${field.value}`)
                                              : t("field.industry.placeholder")}
                                          </span>
                                          <ChevronDown
                                            className="ml-2 size-4 opacity-70 shrink-0"
                                            aria-hidden
                                        ...
                                        {INDUSTRY_KEYS.map((key) => (
                                          ...
                                          >
                                            <Check
                                              ...
                                              aria-hidden
                                            />
                                            {EXISTING_VERTICALS.includes(key as IndustryKey)
                                              ? tSolutions(`${key}.title`)
                                              : t(`field.industry.options.${key}`)}
                                          </DropdownMenuItem>
                                        ))}
                                      </DropdownMenuContent>
                                    </DropdownMenu>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Textarea
                                  placeholder={t("field.painPoints.placeholder")}
                                  rows={8}
                                  required
                                  aria-required="true"
                                  {...field}
                ...
                >
                  ...
                  >
                    <h3 className="mb-4 text-xl font-semibold">{t("report.title")}</h3>
