							...
							<div className='grid items-start gap-4 sm:grid-cols-2'>
													...
													<DropdownMenu>
														...
														<DropdownMenuContent className='w-[--radix-dropdown-menu-trigger-width] min-w-56'>
															<DropdownMenuItem
																...
																aria-selected={(field.value ?? '') === '' ? 'true' : undefined}
															>
																<Check
															...
															{VERTICAL_KEYS.map((key) => (
																<DropdownMenuItem
																	...
																	aria-selected={field.value === key ? 'true' : undefined}
																>
																	<Check
								...
								<FormField
									...
									render={({ field }) => {
										...
										const triggerLabel = () => {
											if (!values || values.length === 0) return t('field.automation.placeholder')
											if (values.length === 1) return tAutomations(`${values[0]}.title`)
														...
														<DropdownMenu>
															...
															<DropdownMenuContent className='w-[--radix-dropdown-menu-trigger-width] min-w-56'>
																<DropdownMenuItem onClick={clear}
																                  aria-selected={values.length === 0 ? 'true' : undefined}>
																	<Check className={`mr-2 size-4 ${values.length === 0 ? 'opacity-100' : 'opacity-0'}`}
																	       aria-hidden/>
